...
/*
 * YB_TODO_PG19MERGE: PG18+ commit 0a20ff54f5e moved per-GUC check/assign/show
 * hook decls into guc_hooks.h, enum-options arrays / GUC globals / display-name
 * arrays into guc_tables.c. YB forward decls remain inline below (build-
 * equivalent); migrate to a YB header for convention parity in a follow-up.
 * YB enum-options arrays, GUC globals, and the PGC_OID "oid" entry in
 * config_type_names have been ported to guc_tables.c.
 */
 ...
 * PG's unified ConfigureNames[] inside build_guc_variables. PG GUCs that YB
 * modifies have those modifications applied directly to the corresponding
 * .dat entries. The ConfigureNamesOid[] array is YB-only because
 * guc_parameters.dat does not yet model PGC_OID.
 */
...
build_guc_variables(void)
{
	...
	for (int i = 0; ConfigureNamesOid[i].gen.name; i++)
	{
		ConfigureNamesOid[i].gen.vartype = PGC_OID;
		num_vars++;
	}
...
#define YB_REGISTER_GUC_ARRAY(arr) \
	...
	{ \
		struct config_generic *gucvar = &arr[i].gen; \
		hentry = (GUCHashEntry *) hash_search(guc_hashtab, &gucvar->name, \
											  HASH_ENTER, &found); \
		Assert(!found); \
		hentry->gucvar = gucvar; \
	}
	YB_REGISTER_GUC_ARRAY(ConfigureNamesBool);
	YB_REGISTER_GUC_ARRAY(ConfigureNamesInt);
	YB_REGISTER_GUC_ARRAY(ConfigureNamesOid);
	YB_REGISTER_GUC_ARRAY(ConfigureNamesReal);
