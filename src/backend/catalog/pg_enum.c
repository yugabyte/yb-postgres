...
restart:
	...
	if (IsBinaryUpgrade || yb_binary_restore)
	{
		...
		/*
		 * YB: For backward compatibility, log a warning messages. Backup
		 * generated by old ysql_dump would not have emitted a query such as
		 * SELECT
		 * pg_catalog.yb_binary_upgrade_set_next_pg_enum_sortorder('2'::real);
		 * to set the enum sortorder. We do not want to fail the restore
		 * because of that.
		 */
