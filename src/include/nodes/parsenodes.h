...
typedef struct RangeTblEntry
{
	...
	/* YB */
	/* alias to use for hinting - unique across a query */
	char	   *ybHintAlias pg_node_attr(query_jumble_ignore);
	/* unique identifer (across all blocks) for a base rel - starting at '1' */
	uint32		ybUniqueBaseId pg_node_attr(query_jumble_ignore);
	char	   *ybScannedObjectName pg_node_attr(query_jumble_ignore);
	char	   *ybSchemaName pg_node_attr(query_jumble_ignore);
} RangeTblEntry;
...
typedef struct Constraint
{
	 ...
	 */
	List	   *yb_index_params pg_node_attr(query_jumble_ignore);	/* IndexElem nodes of UNIQUE or PRIMARY KEY constraint */
} Constraint;
